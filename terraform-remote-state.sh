...
function usage() {
  ...
  echo "Usage: terraform.sh [module_path] [bucket_region] [bucket_name] [application] [environment] [aws_cli_profile]"
  echo
  echo "module_path:"
  ...
  echo " - prod"
  echo
  echo "AWS CLI Profile:"
  echo " - If you have multiple AWS accounts, you can pass in your CLI profile"
}
...
if ! aws s3 ls $terraform_bucket_name > /dev/null 2>&1; then
  echo "State bucket does not exist, creating $terraform_bucket_name..."
  aws s3api create-bucket --bucket $terraform_bucket_name
